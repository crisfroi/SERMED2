...
export function ImportarFichajesPanel() {
  ...
  // --- MODIFICACIÓN CLAVE: CONSULTA DIRECTA CON JOIN/RELACIÓN PARA OBTENER EL NOMBRE ---
  const { data: mappings = [] } = useQuery<MappedEmployeeWithDetails[]>({
    queryKey: ['device-mappings', selectedDevice, 'importar', 'with_name'],
    queryFn: async () => {
      if (!selectedDevice) return [];

      // Consulta con select anidado para obtener el nombre completo
  ...
  // El Set se usa para contar cuántos registros tienen mapeo (EnNo)
  ...
  const enNoToProfNameMap = useMemo(() => {
    ...
    mappings.forEach((m) => {
      ...
      // Solo mapeamos si el nombre existe
      if (profName) {
  ...
  const previewRowsWithMapping = useMemo<EnrichedFichajePreviewRow[]>(() => {
    return previewRows.map((row) => ({
      ...row,
      // NORMALIZACIÓN: Usamos row.enNo.trim() para buscar en el mapa
        ...
        <CardContent className="space-y-6">
          ...
          {previewRows.length ? (
            <Alert variant={unmatchedPreview.length ? 'destructive' : 'default'}>
              ...
              <AlertTitle>
                {/* Ahora el contador debería ser correcto */}
          ...
          {/* Se pasa la nueva lista enriquecida con el nombre_completo */}
