...
// --- INTERFACES GENERALES (sin cambios) ---
...
// ----------------------------------------------------------------------
// 🚨 FUNCIÓN CRÍTICA PARA NORMALIZAR EL EN_NO A SU FORMA CANÓNICA 🚨
// Convierte '0011', '11 ', o simplemente '11' a la cadena CANÓNICA '11'
// para asegurar la coincidencia en la búsqueda.
// ----------------------------------------------------------------------
const normalizeEnNo = (enNo: string | number | null | undefined): string | null => {
  if (!enNo) return null;
  // 1. Limpiar: Eliminar espacios y no-dígitos.
  const str = String(enNo).trim().replace(/\D/g, '');
  if (!str) return null;
  // 2. Normalizar a número y volver a string. Esto elimina ceros iniciales
  // y asegura una representación canónica, evitando problemas de colación/padding.
  const num = parseInt(str, 10);
  // Si la conversión falla, retorna null. Si es válido, retorna la cadena numérica limpia.
...
// ----------------------------------------------------------------------
// ... (useDispositivosFichaje sin cambios) ...
...
export function useDispositivosFichaje() {
  ...
  const upsertMapping = async (
    ...
    // NOTA: ASUMIMOS que el 'en_no' que llega aquí desde el componente (al crear/editar un mapeo)
    // ya ha sido normalizado o es el valor canónico que queremos. Lo importante es que
    // la base de datos almacene el valor que sea más consistente con la importación.
    // Dado que hemos normalizado la importación para usar el valor CANÓNICO,
    // nos aseguramos de que el valor insertado/actualizado aquí sea el mismo.
    const canonicalEnNo = normalizeEnNo(en_no);
    if (!canonicalEnNo) throw new Error('Número de empleado no válido.');
    ...
    if (existingMap) {
      // 2. Si existe, ACTUALIZAMOS el EN_NO con la forma CANÓNICA
      const { data: updatedData, error: updateError } = await supabase
        .from('empleado_dispositivo_map')
        .update({ en_no: canonicalEnNo }) // Usamos la forma CANÓNICA
      ...
      toast({ title: 'Mapeo actualizado', description: `${canonicalEnNo} asignado` });
      return updatedData as EmpleadoDispositivoMap;
    } else {
      // 3. Si no existe, INSERTAMOS un nuevo mapeo con la forma CANÓNICA
      const { data: insertedData, error: insertError } = await supabase
        .from('empleado_dispositivo_map')
        .insert({
          id_profesional: id_profesional,
          en_no: canonicalEnNo, // Usamos la forma CANÓNICA
      ...
      toast({ title: 'Mapeo guardado', description: `${canonicalEnNo} asignado` });
...
// ----------------------------------------------------------------------
// ... (getProfessionalSchedule, fetchTurno sin cambios) ...
...
// ----------------------------------------------------------------------
// 🛠️ USE ASISTENCIA (Con Normalización de EnNo)
// ----------------------------------------------------------------------
...
export function useAsistencia() {
  ...
  const parseLines = (text: string): LogPayload[] => {
    ...
    for (const raw of lines) {
      ...
      // 🚨 APLICACIÓN DE LA CORRECCIÓN CLAVE: NORMALIZAR EL EN_NO A CANÓNICO 🚨
      const sanitized_en_no = normalizeEnNo(en_no_raw);

      entries.push({
        id_profesional: null,
        en_no: sanitized_en_no, // Usar el valor canónico ('11')
        tm_no,
        inout,
        mode,
        fecha_hora,
    ...
    return entries.filter(e => e.en_no);
  };
  ...
  const insertLogs = async (deviceId: string, filename: string, logs: LogPayload[]) => {
    ...
    const rows = validLogs.map(l => {
      // Buscamos el mapeo en el cliente usando la comparación CANÓNICA:
      // Log.en_no ('11') comparado con el EnNo de la DB (normalizado).
      // Esto garantiza que si en la DB hay un valor inesperado (e.g., '11 '),
      // la normalización lo limpie a '11' para la comparación.
      const profId = mappings.find(m =>
        l.en_no && normalizeEnNo(m.en_no) === l.en_no
      )?.id_profesional || null;

      return {
        id_profesional: profId,
        id_dispositivo: deviceId,
        en_no: l.en_no, // Guardamos el valor canónico en el log de asistencia
        tm_no: l.tm_no,
        inout: l.inout,
        mode: l.mode,
  ...
  const importFile = async (deviceId: string, file: File) => {
    ...
    try {
      ...
      const parsed = parseLines(text); // parseLines ahora usa normalizeEnNo
    ...
    try {
      ...
      for (const sheetName of wb.SheetNames) {
        ...
        const parsed: LogPayload[] = rows.map((r) => {
          ...
          // 🚨 APLICACIÓN DE LA CORRECCIÓN CLAVE: NORMALIZAR EL EN_NO DEL FICHAJE XLS 🚨
          const sanitized_en_no = normalizeEnNo(enRaw);
          ...
          return {
            id_profesional: null,
            en_no: sanitized_en_no, // Usar el valor canónico
  ...
  const importPersonalXls = async (deviceId: string, file: File, centerId?: string | null) => {
    ...
    try {
      ...
      (profs || []).forEach((p: any) => {
        ...
        // Normalizamos los EnNo de la base de datos con la nueva lógica (a canónico)
        const normalized = normalizeEnNo(raw);
        if (normalized) {
      ...
      rows.forEach((r, index) => {
        ...
        // 🚨 APLICACIÓN DE LA CORRECCIÓN CLAVE: NORMALIZAR EL EN_NO DEL ARCHIVO PERSONAL.XLS 🚨
        // Usamos la lógica CANÓNICA
        const enNo = normalizeEnNo(rawEmp);
        ...
        // Buscamos usando el EnNo CANÓNICO
        ...
        // Guardamos el mapeo con el EnNo CANÓNICO
      ...
      // El `onConflict: 'id_dispositivo,en_no'` funciona porque ya normalizamos `en_no` a su forma canónica.
