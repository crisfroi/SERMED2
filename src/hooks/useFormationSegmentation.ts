...
// CAMBIO: Ahora acepta institutionId en lugar de institution
export const useFormationSegmentation = (params: { country?: string | null; institutionId?: string | null }) => {
    // CAMBIO: Desestructuramos institutionId
    const { country, institutionId } = params;

    return useQuery({
        // CAMBIO: Usamos institutionId en la queryKey
        queryKey: ["formationSegmentation", country || null, institutionId || null],
        // CAMBIO: Usamos institutionId en el enabled
        enabled: Boolean(country || institutionId),
...
            }
            // CAMBIO: Filtramos por institucion_formacion_id_1 usando el institutionId
            if (institutionId) {
                query = query.eq(`institucion_formacion_id_1`, institutionId);
            }
