...
function readMigrationFile(filename: string): string {
  ...
  lines
    ...
    .forEach((line) => {
      if (line.trim()) {
        log(`   ${line.substring(0, 80)}${line.length > 80 ? "..." : ""}`, "yellow");
      }
    });
...
async function checkMigrationStatus(
  ...
  try {
    ...
    log(`⚠️  Error: ${error instanceof Error ? error.message : "Desconocido"}`, "yellow");
    return false;
  }
}
...
async function createMigrationTrackingTable(supabase: any) {
  try {
    ...
    log(`\n📋 Creando tabla de tracking de migraciones...`, "blue");
    ...
    if (error && !error.message.includes("already exists")) {
      log(`⚠️  No se pudo crear tabla (puede que ya exista): ${error.message}`, "yellow");
    } else {
      log(`   ✅ Tabla creada o ya existe`, "green");
    }
  } catch (error) {
    log(`   ⚠️  ${error instanceof Error ? error.message : "Error desconocido"}`, "yellow");
  }
}
...
async function main() {
  try {
    log("\n" + "=".repeat(70), "bright");
    log("  🧪 PRUEBA: LECTURA Y APLICACIÓN DE MIGRACIONES EN SUPABASE", "bright");
    ...
    if (!isApplied) {
      log(`\n⚠️  La migración NO está aplicada. ¿Aplicarla ahora?`, "yellow");
    ...
    log(`✅ Estado: ${isApplied ? "YA APLICADA" : "RECIÉN REGISTRADA"}`, "green");

    log("\n💡 PRÓXIMOS PASOS:", "blue");
    log(`   1. Verificar en Supabase Dashboard`, "yellow");
    log(
      `   2. Ver tabla 'schema_migrations': https://app.supabase.com/project/wdieynendfjbkbhfovrx/editor`,
      "yellow"
    );
